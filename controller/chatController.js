...
const saveChat = async ( conversation_id, receiver_id, sender_id, message ) => {
    try {
...
const getChat = async (conversation_id) => {
    try {
        const[get_chat] = await pool.query(`SELECT m.message_id,m.receiver_id, rup.username AS receiver,m.sender_id,sup.username AS sender, sup.profile_image AS profile_img, m.message, m.created_at, m.updated_at 
